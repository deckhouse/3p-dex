...
func (h requestContextHandler) Handle(ctx context.Context, record slog.Record) error {
	if v, ok := ctx.Value(authflow.RequestKeyRemoteIP).(string); ok {
		record.AddAttrs(slog.String(string(authflow.RequestKeyRemoteIP), v))
	}

	if v, ok := ctx.Value(authflow.RequestKeyRequestID).(string); ok {
		record.AddAttrs(slog.String(string(authflow.RequestKeyRequestID), v))
	}
