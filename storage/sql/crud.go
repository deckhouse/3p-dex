	...
	return c.ExecTx(func(tx *trans) error {
		...
		_, err = tx.Exec(`
			...
			set
				...
				auth_policy = $7
			where id = $8;
		`, nc.Secret, encoder(nc.RedirectURIs), encoder(nc.TrustedPeers), nc.Public, nc.Name, nc.LogoURL, encoder(nc.AuthPolicy), id,
		)
		if err != nil {
...
func (c *conn) CreateClient(ctx context.Context, cli storage.Client) error {
	_, err := c.Exec(`
		insert into client (
			id, secret, redirect_uris, trusted_peers, public, name, logo_url, auth_policy
		)
		values ($1, $2, $3, $4, $5, $6, $7, $8);
	`,
		cli.ID, cli.Secret, encoder(cli.RedirectURIs), encoder(cli.TrustedPeers),
		cli.Public, cli.Name, cli.LogoURL, encoder(cli.AuthPolicy),
	)
	if err != nil {
		if c.alreadyExistsCheck(err) {
			return storage.ErrAlreadyExists
...
func scanClient(s scanner) (cli storage.Client, err error) {
	err = s.Scan(
		...
		&cli.Public, &cli.Name, &cli.LogoURL, decoder(&cli.AuthPolicy),
	)
	if err != nil {
		if err == sql.ErrNoRows {
