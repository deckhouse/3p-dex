...
// SessionConfig, PKCEConfig and MFAProvider are defined by the authflow package
// (which owns the interactive flow) and aliased here so the public server.Config
// API stays unchanged.
type (
	SessionConfig = authflow.SessionConfig
	PKCEConfig    = authflow.PKCEConfig
	MFAProvider   = authflow.MFAProvider
)
...
func newServer(ctx context.Context, c Config) (*Server, error) {
	...
	handlerWithHeaders := func(handlerName string, handler http.Handler) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			rCtx := r.Context()
			rCtx = authflow.WithRequestID(rCtx)

			if c.RealIPHeader != "" {
				realIP, err := parseRealIP(r)
				if err == nil {
					rCtx = authflow.WithRemoteIP(rCtx, realIP)
				}
			}
