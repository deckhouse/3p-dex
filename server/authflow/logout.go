...
func (h *Handler) handleLogout(w http.ResponseWriter, r *http.Request) {
	...
	if userID == "" && connectorID == "" {
		if cookie, err := r.Cookie(h.sessionConfig.CookieName); err == nil && cookie.Value != "" {
			if uid, cid, nonce, err := internal.ParseSessionCookie(cookie.Value, h.sessionConfig.CookieEncryptionKey); err == nil {
...
func (h *Handler) handleLogoutCallback(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || cookie.Value == "" {
		h.renderError(r, w, http.StatusBadRequest, "Missing session cookie.")
		return
	}
